...
def _compute_forces(p, t, fh, min_edge_length, L0mult, opts):
    ...
    if opts["stereo"]:
        ...
        # we need to rectify the force based on the stereographic distortion
        x, y = to_lat_lon(p[:, 0], p[:, 1])
        p0 = np.asarray([x, y]).T
        pp = p0[bars]
        mean_lats = pp[:, :, 1].sum(1) / 2
        # fixing weights across the -180/180 line
        left_lons = pp[:, 0, 0]
        right_lons = pp[:, 1, 0]
        ix1 = left_lons - right_lons > 180
        ix2 = left_lons - right_lons < -180
        pp[:, 1, 0][ix1] -= 360
        pp[:, 1, 0][ix2] += 360
        # computing corrected segments lengths
        hbars = fh(pp.sum(1) / 2)
        # applying distortion corrections
        hbars *= _stereo_distortion(mean_lats)
        hbars[np.isnan(hbars)] = 0
        hbars[np.isinf(hbars)] = 999999  # _FILL
    else:
